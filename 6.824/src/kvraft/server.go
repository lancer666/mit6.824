...
func (kv *KVServer) Get(args *GetArgs, reply *GetReply) {
	...
	//createNotifych相当于为该index的cmd创建了一个容量为1的channel（notifych），当applyMsg函数执行时，reply会被放到channel之中，之后Get方法只需要等待消息（判断超时）即可。
